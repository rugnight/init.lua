...
### プラグイン一覧の保守 ⭐ **重要**
当設定では[PLUGINS.md](PLUGINS.md)でプラグイン一覧を管理しています。
**プラグインの追加・削除・変更時には必ずこのドキュメントを最新の状態に保つよう務めてください。**

#### 更新が必要な場合
1. **プラグイン追加**: `lua/plugins/`に新しい`.lua`ファイルを作成した場合
2. **プラグイン削除**: 既存のプラグインファイルを削除した場合
3. **プラグイン変更**: リポジトリURLや設定を大幅に変更した場合
4. **定期レビュー**: 月1回程度、実際の設定との整合性を確認

#### 保守手順
1. `lua/plugins/`ディレクトリ内のファイル構成を確認
2. [PLUGINS.md](PLUGINS.md)の内容と比較
3. 変更があれば総プラグイン数・最終更新日・該当箇所を更新
4. このCLAUDE.mdの主要プラグインリストも必要に応じて更新

### ドキュメント構成
- **CLAUDE.md**: 設定概要・機能説明・使用方法（このファイル）
- **PLUGINS.md**: 全プラグインの詳細一覧・カテゴリ分類
- **GEMINI.md**: Gemini AI連携設定
- **lazy-lock.json**: プラグインバージョンロック（自動管理）

正確なドキュメント管理により、設定の可視性・保守性・新規導入時の理解促進を図ります。